...
class Drone_Hover_Adversary(DroneHoverFreeEnv):
    def __init__(self,
                 aggregate_phy_steps=2,  # sub-steps used to calculate motor dynamics
                 control_mode='PWM',
                 **kwargs):
        super(Drone_Hover_Adversary, self).__init__(
            aggregate_phy_steps=aggregate_phy_steps,
            control_mode=control_mode,
    ...
    """
    def _setup_task_specifics(self):
        super(Drone_Hover_Adversary, self)._setup_task_specifics()
    ...
    def render(self, mode="human"):
        super(Drone_Hover_Adversary, self).render(mode)

    """
    Joe: This is for adversary agent, crash means done 
    """
    def compute_done(self) -> bool:
        """ Note: the class is wrapped by Gym's Time-wrapper, which returns
        done=True when T >= time_limit."""
    ...
    """
    Joe: This is for adversary agent, crash means done, encourage the agent to crash 
    """ 
    def compute_reward(self, action) -> float:  # Hanyang: redesigning
        # Determine penalties
        act_diff = action - self.drone.last_action
