...
class IWPGAlgorithm_Separate(core.OnPolicyGradientAlgorithm):
    def __init__(
            self,
            actor: str,
            ac_kwargs: dict,
        ...
        self.params = locals()  # Return a dictionary containing the current scope's local variables.
        # Hanyang: take the env instace as input directly instead of env_id
        self.env = env
    ...
    def algorithm_specific_logs(self):
        ...
        # if self.env_id == 'DroneHoverFixedDistbEnvry-v0':
    ...
    def roll_out(self) -> None:
        ...
        for t in range(self.local_steps_per_epoch):  # self.local_steps_per_epoch = steps_per_epoch (32*1000) // mpi_tools.num_procs()
            a, v, logp = self.ac.step(
                torch.as_tensor(o, dtype=torch.float32))  # Hanyang: a is the action generated by the policy network, v is the value generated by the value network
    ...
    def update_value_net(self, data: dict) -> None:
        mbs = int(self.local_steps_per_epoch // self.num_mini_batches)  # mini-batch size = 16, mbs is the number of mini-batches
    ...
    def val_once(self, episode):
        ...
        while not done:
            frames.append(self.env.capture_image())
...
#TODO: Hanyang: need to reconsctruct the get_alg function to take env as input
def get_alg(env, **kwargs) -> core.Algorithm:
    return IWPGAlgorithm_Separate(env=env, **kwargs)


# compatible class to OpenAI Baselines learn functions
#TODO: Hanyang: need to reconsctruct the learn function to take env as input
def learn(env, **kwargs) -> tuple:
    defaults = utils.get_separate_defaults_kwargs(alg='iwpg_separate')
    defaults.update(**kwargs)
    alg = IWPGAlgorithm_Separate(env=env, **defaults)
    ac, env = alg.learn()
    return ac, env
