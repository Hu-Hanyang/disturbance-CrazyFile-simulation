...
from phoenix_drone_simulation.envs.hover_distb import DroneHoverFixedDistbEnv, DroneHoverBoltzmannDistbEnv
...
env = DroneHoverFixedDistbEnv(observation_noise=0.5, enable_reset_distribution=False)
...
print(f"The enable reset distribution is {env.enable_reset_distribution}")
...
while num < num_gifs:
    ...
    for _ in range(max_steps):
        ...
        if done or steps>=max_steps:
            ...
            create_gif(frames[num], f'env_gif{num}-motor{motor}-obs_noise{env.observation_noise}-{steps}steps.gif', duration=0.1)
